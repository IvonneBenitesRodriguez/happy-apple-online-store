...
- Helper specs (dashboard "thought of the day" rotation).<br/>
...
Passwords hashed with BCrypt via has_secure_password; no plaintext password is ever stored or logged.
...
```
2. Access Control — IDOR Protection ⚠️
...
config.force_ssl = true and config.assume_ssl = true enabled in production, enforcing HTTPS for all traffic.
