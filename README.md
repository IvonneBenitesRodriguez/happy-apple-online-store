...
This project includes multiple security artifacts to support transparency and auditability:
-**`brakeman_report.html`** - Static Application Security Testing (SAST) report generated with Brakeman, scanning for common Rails vulnerabilities (0 warnings).<br/>
-**`bom.xml`** / **`bom-modern.json`** - Software Bill of Materials (SBOM) in **CycloneDX** format, listing all project dependencies and their versions.<br/>
-**`sbom.spdx.json`** - Software Bill of Materials (SBOM) in **SPDX** format, natively generated by Github's Dependency Graph, offering a second industry-standard SBOM for broader tooling compatibility. 
